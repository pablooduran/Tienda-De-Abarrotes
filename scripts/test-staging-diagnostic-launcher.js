...
function main() {
  ...
  try {
    ...
    check('frozen-regression', 'CONNECTION_OR_CONFIGURATION_FAILURE PREREQUISITE_LOCAL');
    check('read-failure', 'CONNECTION_OR_CONFIGURATION_FAILURE DATABASE_NOT_FOUND_OR_PERMISSION');
    check('close-failure', 'CONNECTION_OR_CONFIGURATION_FAILURE NETWORK_TIMEOUT_OR_ALLOWLIST');
    for (const [code, cause] of Object.entries({
      ...
      check('connection:' + code, 'CONNECTION_OR_CONFIGURATION_FAILURE ' + cause);
    }
    check('connection:TYPE_ERROR', 'CONNECTION_OR_CONFIGURATION_FAILURE PREREQUISITE_LOCAL');
    check('connection:UNLISTED_ERROR', 'CONNECTION_OR_CONFIGURATION_FAILURE UNKNOWN_SAFE_FAILURE');
    for (const scenario of ['protocol-missing', 'protocol-unknown', 'protocol-duplicate', 'protocol-mixed']) {
      check(scenario, 'CONNECTION_OR_CONFIGURATION_FAILURE PREREQUISITE_LOCAL');
    }
