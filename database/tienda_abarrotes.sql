...
-- SUBSCRIPTION_LIFECYCLE_SCHEMA_START
CREATE TABLE IF NOT EXISTS suscripcionTienda (
  ...
  estado ENUM('pendiente','activa','gracia','vencida','suspendida','cancelada') NOT NULL DEFAULT 'pendiente',
  fechaInicio DATETIME NOT NULL,
  fechaFin DATETIME NOT NULL,
  fechaFinGracia DATETIME NULL,
  suspendidaEn DATETIME NULL,
  reactivadaEn DATETIME NULL,
  canceladaEn DATETIME NULL,
  motivoTransicion ENUM(
  ...
  idPlanSiguiente INT NULL,
  fechaAplicacionPlanSiguiente DATETIME NULL,
  planCodigoSnapshot VARCHAR(50)
    CHARACTER SET ascii COLLATE ascii_bin NOT NULL,
  planNombreSnapshot VARCHAR(100) NOT NULL,
  tipoPeriodoSnapshot ENUM('mensual','anual','personalizada') NOT NULL,
  duracionDiasSnapshot INT UNSIGNED NOT NULL,
  precioReferenciaSnapshot DECIMAL(10,2) NOT NULL,
  ...
  creadoEn DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  actualizadoEn DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  CONSTRAINT uq_suscripcion_tienda_id UNIQUE (idTienda,idSuscripcion),
  KEY idx_suscripcion_tienda_estado_fechas (idTienda, estado, fechaInicio, fechaFin),
  KEY idx_suscripcion_tienda_gracia (idTienda,estado,fechaFin,fechaFinGracia),
  KEY idx_suscripcion_plan (idPlan),
  KEY idx_suscripcion_plan_siguiente (idPlanSiguiente,fechaAplicacionPlanSiguiente),
  ...
  CONSTRAINT fk_suscripcion_plan_siguiente FOREIGN KEY (idPlanSiguiente)
  ...
  CONSTRAINT chk_suscripcion_fechas_ciclo CHECK (
  ...
  CONSTRAINT chk_suscripcion_plan_siguiente CHECK (
  ...
  CONSTRAINT chk_suscripcion_snapshot CHECK (
...
CREATE TABLE IF NOT EXISTS historialSuscripcionTienda (
  ...
  idTienda INT NOT NULL,
  idSuscripcion INT NOT NULL,
  estadoAnterior ENUM('pendiente','activa','gracia','vencida','suspendida','cancelada') NULL,
  estadoNuevo ENUM('pendiente','activa','gracia','vencida','suspendida','cancelada') NOT NULL,
  tipoOperacion ENUM(
  ...
  KEY idx_historialSuscripcion_tienda_fecha (idTienda,creadoEn,idHistorialSuscripcion),
  ...
  CONSTRAINT chk_historialSuscripcion_actor CHECK (
...
CREATE TABLE IF NOT EXISTS operacionSuscripcionTienda (
  ...
  UNIQUE KEY uq_operacionSuscripcion_clave (idTienda,tipoOperacion,claveHash),
  KEY idx_operacionSuscripcion_estado_expira
    (estado,expiraEn,idOperacionSuscripcion),
  KEY idx_operacionSuscripcion_resultado (idTienda,idSuscripcionResultado),
  CONSTRAINT chk_operacionSuscripcion_hashes CHECK (
    claveHash REGEXP '^[0-9a-f]{64}$'
    AND huellaSolicitud REGEXP '^[0-9a-f]{64}$'
  ),
  CONSTRAINT chk_operacionSuscripcion_fechas CHECK (
    expiraEn>creadoEn
    AND (
  ...
  CONSTRAINT fk_operacionSuscripcion_tienda FOREIGN KEY (idTienda)
...
-- SUBSCRIPTION_LIFECYCLE_SCHEMA_END
...
-- SUBSCRIPTION_LIFECYCLE_SEED_START
INSERT INTO suscripcionTienda
  (idTienda,idPlan,tipo,estado,fechaInicio,fechaFin,fechaFinGracia,
   ...
   renovacionAutomatica,observacion,creadoPor)
SELECT t.idTienda, p.idPlan, 'cortesia', 'activa', @fecha_local_instalacion,
       DATE_ADD(@fecha_local_instalacion, INTERVAL 3650 DAY), NULL,
       NULL,p.codigo,p.nombre,'personalizada',3650,p.precioMensual,
       p.limitePropietarios,p.limiteProductos,p.limiteClientes,p.limiteProveedores,0,
...
-- SUBSCRIPTION_LIFECYCLE_SEED_END
