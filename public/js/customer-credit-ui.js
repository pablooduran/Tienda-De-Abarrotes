  ...
  function create(deps) {
    ...
    function customerActions(customer) {
      if (!customer.activo) return `<div class="actions customer-actions">
        ...
        ${readOnly() ? `<button type="button" class="small secondary" data-customer-statement="${customer.idCliente}">Estado de cuenta</button>` : `<details class="row-actions"><summary>Más opciones</summary><div class="row-actions-menu"><button type="button" class="small secondary" data-customer-statement="${customer.idCliente}">Estado de cuenta</button><button type="button" class="small" data-customer-restore="${customer.idCliente}">Restaurar cliente</button><button type="button" class="small secondary secondary-actions-close" data-secondary-actions-close>Cerrar</button></div></details>`}
      </div>`;
      return `<div class="actions customer-actions">
        ...
        ${readOnly() && !(can('recordatorios_fiado') && customer.aceptaRecordatorios) ? `<button type="button" class="small secondary" data-customer-statement="${customer.idCliente}">Estado de cuenta</button>` : `<details class="row-actions"><summary>Más opciones</summary><div class="row-actions-menu">${!readOnly() ? `<button type="button" class="small secondary" data-customer-edit="${customer.idCliente}">Editar</button>` : ''}<button type="button" class="small secondary" data-customer-statement="${customer.idCliente}">Estado de cuenta</button>${can('recordatorios_fiado') && customer.aceptaRecordatorios ? `<button type="button" class="small secondary" data-customer-whatsapp="${customer.idCliente}">WhatsApp</button>` : ''}${!readOnly() ? `<button type="button" class="small danger" data-customer-hide="${customer.idCliente}">Ocultar cliente</button>` : ''}<button type="button" class="small secondary secondary-actions-close" data-secondary-actions-close>Cerrar</button></div></details>`}
      </div>`;
    }
    ...
    async function renderCustomers() {
      ...
      try {
        ...
        const secondaryActions = [
          can('segmentacion_clientes') ? '<button type="button" class="secondary" data-customer-segmentation>Segmentación</button>' : '',
          can('limites_credito') ? '<button type="button" class="secondary" data-credit-config>Configurar crédito</button>' : '',
          can('exportacion_clientes_fiados') ? `<button type="button" class="secondary" data-export-customers ${readOnly() ? 'disabled title="La suscripción debe estar activa para exportar."' : ''}>Exportar clientes</button>` : ''
        ].filter(Boolean);
        const secondaryMenu = secondaryActions.length > 1
          ? `<details class="inventory-secondary-actions"><summary>Más opciones</summary><div>${secondaryActions.join('')}<button type="button" class="secondary secondary-actions-close" data-secondary-actions-close>Cerrar</button></div></details>`
        ...
        view.innerHTML = `
          <div class="credit-heading"><div><span class="eyebrow">Clientes</span><h3>Relaciones claras, cuentas al dia</h3><p>Consulta deuda, credito y actividad sin perder el historial.</p></div>
            <div class="actions">${!readOnly() ? '<button type="button" data-new-customer>Agregar cliente</button>' : ''}${secondaryMenu}</div></div>
