(function initializePaymentSubscriptionAdminUi(global) {
  ...
  function create() {
    ...
    async function saveMethod(form, button) { button.disabled = true; try { const body = { activo: form.elements.activo.checked, visiblePropietario: form.elements.visiblePropietario.checked, instrucciones: form.elements.instrucciones.value || null }; await request(`/api/admin/pagos-suscripcion/metodos/${encodeURIComponent(form.dataset.reference)}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json', 'Idempotency-Key': operationKey('payment-method') }, body: JSON.stringify(body) }); await loadConfiguration(); } catch (error) { global.alert(error.message); } finally { button.disabled = false; } }
    elements.rateForm.addEventListener('submit', async (event) => { event.preventDefault(); const button = elements.rateForm.querySelector('button'); button.disabled = true; try { await request('/api/admin/pagos-suscripcion/tipos-cambio', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Idempotency-Key': operationKey('payment-rate') }, body: JSON.stringify(Object.fromEntries(new FormData(elements.rateForm).entries())) }); elements.rateForm.reset(); await loadConfiguration(); } catch (error) { global.alert(error.message); } finally { button.disabled = false; } });
