(function initializePaymentSubscriptionUi(global) {
  ...
  function create({ root, api = null } = {}) {
    ...
    function requestRow(item) {
      return `<article class="payment-request-row"><div><strong>${escapeHtml(item.plan.nombre)}</strong><span>${escapeHtml(item.operacion.replaceAll('_', ' '))} · ${escapeHtml(item.periodo)}</span></div><div><strong>${money(item.montoBOB, 'BOB')}</strong><span>USD ${escapeHtml(item.precioBaseUSD)}</span></div><div><span class="payment-state" data-state="${escapeHtml(item.estado)}">${escapeHtml(label(item.estado))}</span><span>${escapeHtml(date(item.venceEn))}</span></div><button type="button" class="button-link" data-request-detail="${escapeHtml(item.referencia)}">Ver detalle</button></article>`;
    }
    ...
    async function quoteOrCreate(event) {
      ...
      const body = Object.fromEntries(new FormData(form).entries()); button.disabled = true;
      ...
      } catch (error) { feedback.textContent = error.message || 'No se pudo completar la operación.'; } finally { button.disabled = false; }
    }
    ...
    function renderShell() {
      root.innerHTML = `<div class="payment-subscription-shell"><div class="payment-section-heading"><div><p class="subscription-eyebrow">Pagos manuales</p><h2>Renovar o cambiar plan</h2><p>Los montos se calculan de forma segura cuando solicitas una cotización.</p></div></div><form data-payment-form data-payment-action="quote" class="payment-config-form"><label><span>Plan</span><select name="plan" required></select></label><label><span>Operación</span><select name="operacion" required></select></label><label><span>Periodo</span><select name="periodo" required></select></label><label><span>Método</span><select name="metodo" required></select></label><p data-payment-plan-description class="payment-muted"></p><div class="payment-form-actions"><button type="submit" class="button-link">Cotizar</button><button type="button" class="button-link secondary" data-create-payment>Crear solicitud</button></div></form><div data-payment-quote></div><p data-payment-feedback role="status" aria-live="polite"></p><section class="payment-request-list" aria-labelledby="payment-request-list-title"><div class="payment-section-heading"><h3 id="payment-request-list-title">Mis solicitudes</h3><div><button type="button" class="button-link" data-payment-previous>Anterior</button><span data-payment-page></span><button type="button" class="button-link" data-payment-next>Siguiente</button></div></div><div data-payment-requests></div></section><div data-payment-detail></div></div>`;
