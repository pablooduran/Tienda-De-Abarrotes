...
| --- | --- | --- | --- | --- | --- | --- |
| Autenticacion y sesiones | `routes/`, `middleware/`, `config/`, servicios de sesion | `node --check`; `npm.cmd run test:local-startup` | `npm.cmd run test:session-revocation`; `npm.cmd run db:check-session-security`; `npm.cmd run test:timezone-tls` | `npm.cmd run test:web-security`; `npm.cmd run test:operational-health` | Fixtures segun arnes; no navegador salvo cambio UI | Cerrar servidor propio; detener ante secretos o sesion cruzada |
