...
| --- | --- | --- | --- | --- | --- | --- |
| Autenticacion, sesiones, registro publico y verificacion | `routes/`, `middleware/`, `config/`, servicios de sesion, registro y verificacion | `node --check`; `npm.cmd run test:public-registration`; `npm.cmd run test:email-verification` | `npm.cmd run test:session-revocation`; `npm.cmd run db:check-session-security`; `npm.cmd run test:timezone-tls`; tenant y suscripciones si cambia alta | `npm.cmd run test:web-security`; `npm.cmd run test:operational-health` | Registro y verificacion usan bases temporales propias; no navegador en SAAS-A1/A2 | Cerrar solo temporal propio; detener ante secreto, sesion cruzada, token persistido en claro o alta parcial |
