...
| --- | --- | --- | --- | --- | --- | --- |
| Autenticacion, sesiones y registro publico | `routes/`, `middleware/`, `config/`, servicios de sesion y registro | `node --check`; `npm.cmd run test:local-startup`; `npm.cmd run test:public-registration` | `npm.cmd run test:session-revocation`; `npm.cmd run db:check-session-security`; `npm.cmd run test:timezone-tls`; tenant y suscripciones si cambia alta | `npm.cmd run test:web-security`; `npm.cmd run test:operational-health` | Registro usa base temporal propia; no navegador en SAAS-A1 | Cerrar solo temporal propio; detener ante secreto, sesion cruzada o alta parcial |
