...
| TECH-028 | Product Analytics | PRODUCT-GROWTH necesita medir uso sin acoplar la aplicacion a un proveedor | alta antes de beta | adaptador desacoplado, eventos, funnels, activacion, retencion, privacidad y proveedor desactivable/no-op local | No | Parcialmente implementado: contrato, allowlist y noop local; PostHog sigue sin contrato ni configuracion | PRODUCT-GROWTH-0A |
| PRODUCT-029 | Activation / Aha Moment | La secuencia producto -> stock -> primera venta es una hipotesis que debe medirse | alta antes de beta | activation rate, time to first sale, D1, D7, D30 y relacion con WELCOME/retencion | No | Hipotesis instrumentable, no validada; medicion remota pendiente | PRODUCT-GROWTH-0A |
| PRODUCT-030 | Churn / Cancelacion | Hace falta observar abandono sin dark patterns ni cambiar el flujo comercial actual | media antes del lanzamiento | inicio de cancelacion, motivo estructurado, comentario opcional, confirmacion, evento analitico y analisis | Probable migracion propia | Implementado parcial: catalogo y contrato analytics; self-service y fuente persistente requieren decision | PRODUCT-GROWTH-0B |
| CHURN-001 | Persistencia de intencion de cancelacion | `motivoTransicion` corresponde a transiciones administrativas y no debe reutilizarse para churn del propietario | alta antes de beta | semantica de fechaFin/gracia, historial auditable, tenant y retencion de comentario | Nueva estructura por definir | Requiere decision del propietario | PRODUCT-GROWTH-0B / Suscripciones |
