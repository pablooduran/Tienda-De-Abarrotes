...
| TECH-028 | Product Analytics | PRODUCT-GROWTH necesita medir uso sin acoplar la aplicacion a un proveedor | alta antes de beta | adaptador desacoplado, eventos, funnels, activacion, retencion, privacidad y proveedor desactivable/no-op local | No | Parcialmente implementado: contrato, allowlist, noop y cinco hechos backend post-commit; `product_created`, proveedor y control operativo siguen pendientes | PRODUCT-GROWTH-0A / 0C1 |
| PRODUCT-029 | Activation / Aha Moment | La secuencia producto -> stock -> primera venta es una hipotesis que debe medirse | alta antes de beta | `product_created` idempotente, activation rate, time to first sale, D1, D7, D30 y relacion con WELCOME/retencion | No | Hipotesis no validada: registro, verificacion, onboarding, stock y venta quedan instrumentados; producto y medicion remota siguen pendientes | PRODUCT-GROWTH-0C1 / futura medicion |
