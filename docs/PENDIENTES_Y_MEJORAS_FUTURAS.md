...
| TECH-028 | Product Analytics | PRODUCT-GROWTH necesita medir uso sin acoplar la aplicacion a un proveedor | alta antes de beta | adaptador desacoplado, eventos, funnels, activacion, retencion, privacidad y proveedor desactivable/no-op local | No | PRODUCT-GROWTH-0 cerrado en alcance local/desacoplado. En el primer piloto puede continuar `noop`: medicion remota, proveedor y control operativo siguen pendientes para beta/escala y no bloquean `PILOT_READY`; el CI remoto reciente no es visible desde el conector | PRODUCT-GROWTH-0 / beta-escala |
| PRODUCT-029 | Activation / Aha Moment | La secuencia producto -> stock -> primera venta es una hipotesis que debe medirse | alta antes de beta | `product_created` idempotente, activation rate, time to first sale, D1, D7, D30 y relacion con WELCOME/retencion | No | Hipotesis no validada: registro, verificacion, onboarding, stock, venta y eventos comerciales complementarios quedan instrumentados; producto, cliente y medicion remota siguen pendientes | PRODUCT-GROWTH-0C1 / 0C2 / futura medicion |
...
PRODUCT-GROWTH-0 queda cerrado dentro del alcance local/desacoplado. No existe
medicion remota, Session Replay ni proveedor analytics activo; PRODUCT-029 sigue
siendo una hipotesis no validada y no se afirman activation rate, retencion D1,
D7 o D30 ni resultados de usuarios reales. `product_created` y
`customer_created` permanecen diferidos por replay seguro; CHURN-001 queda
resuelto como decision documental y su persistencia requiere una migracion
futura, nunca 025. PostHog sigue siendo solo candidato. El siguiente bloque es
`PILOT-READINESS`, que puede comenzar, pero `PILOT_READY` requiere evidencia de
CI y gates exigidos.
...
especifica del piloto, backup, restore probado, integridad de ventas/dinero,
